...
gen<-population[[1]]$`Gender`[seq(1, 250, 5)]
...
tom_res=cbind(order(60*24*times(tom_res1[,3])),order(order(60*24*times(tom_res1[,4]))),tom_res1)
colnames(tom_res)<-c("Tour Position (Gun)","Position (Chip)", "Bib", "Name", "Gun Time", "Chip Time")
tom_res %>%
